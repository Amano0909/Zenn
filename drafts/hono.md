# Hono自分用メモ：Cloudflare Pages FunctionsでHono APIを動かすまで
...
結論からいうと、React + Vite + Cloudflare Pages + Hono APIで考える場合、かなり重要なのはこの1行でした。結構混乱しました。わけわかめ

```ts
export const onRequest = handle(app, "/api");
```

これは、Cloudflare Pages Functionsの入口である `onRequest` に、Honoアプリを接続するためのコードです。

参照元: [Hono - Hono Stacks / With React](https://hono.dev/docs/concepts/stacks)
...
そのため、Hono公式のCloudflare Pages Starterは重要ですが、自分の用途では `c.render()` やJSXレンダリングは最初から使う想定がないでです。
...
Hono公式のHono Stacksでは、React + Cloudflare PagesのAPIサーバー例として、`functions/api/[[route]].ts` にHonoアプリを書き、最後に `export const onRequest = handle(app, '/api')` する構成が紹介されています。

参照元: [Hono - Hono Stacks / With React](https://hono.dev/docs/concepts/stacks)

## `export const onRequest = handle(app, "/api")` の意味
...
"/api"
  Cloudflare Pages側でこのFunctionが担当するURL prefix
```
...
この理解がないと、`app.get('/posts')` と書いたHonoアプリが、なぜCloudflare Pages上で `/api/posts` として動くのか分かりにくいです。
...
つまり、Hono側には `/api` を書いていません。

そこで `handle(app, "/api")` とすることで、Cloudflare Pages側の `/api` 配下のリクエストをHonoアプリに渡す形になります。Hono公式のReact + Cloudflare Pages例でも、`functions/api/[[route]].ts` と `handle(app, '/api')` がセットで使われています。
...
Hono公式のHono Stacksでも、React + Cloudflare Pagesの例として、APIサーバーを `functions/api/[[route]].ts` に書く構成が紹介されています。
...
ただし、Hono側では以下のように `/api` なしで書いています。
...
ただし、この記事の構成では `functions/api/[[route]].ts` と `handle(app, "/api")` を使うため、Hono側では `/api` なしで書くほうが読みやすいです。
...
functions/api/[[route]].ts
  「あるよ。handle(app, "/api") を渡すよ」

Hono
  「じゃあ /posts や /health は自分が処理するよ」
```

この理解があると、Cloudflare Pages上でHono APIが動く理由がかなりみえました。
